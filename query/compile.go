...
// 表示经过一些初始处理的SELECT语句，以确定它是否有效，并对AST进行一些初始修改。
type compiledStatement struct {
	...
	// 如果间隔被父级继承，则InheritedInterval标记。如果设置了这个值，那么继承的间隔将不会导致不应该有间隔的查询失败。
	...
	// compiled query.
	// ExtraIntervals是除了TimeRange之外将被读取的额外间隔数。
	// 它是Interval的倍数，并且只适用于有Interval的查询。
	// 它用于扩展映射的分片的TimeRange，以包括派生函数和其他函数使用的额外的非发射间隔。
	// 它将被设置为需要读取的最大额外间隔数，即使它并不适用于所有函数。这个数总是正的。
	// 即使编译后的查询没有间隔，该值也可以设置为非零值。
	ExtraIntervals int
	...
	// 没有聚合函数时设为true，表明只是字段的Projection
	OnlySelectors bool
	...
	// this name.
	// 时间列名，默认"time"，但可以起别名（select time as timestamp）
...
// Compile 对语句中的每个部分（projection、selection）进行提取、变换，放到全局变量
func Compile(stmt *influxql.SelectStatement, opt CompileOptions) (Statement, error) {
	c := newCompiler(opt)
	c.stmt = stmt.Clone()
	// 预处理，对语句中的除了projection部分进行提取，放到全局变量
...
// 从语句中计算当前语句的全局属性： Ascending, Limit, HasTarget, Condition, TimeRange, FillOption
func (c *compiledStatement) preprocess(stmt *influxql.SelectStatement) error {
	...
	if c.TimeRange.Max.IsZero() {
		...
		// and usability.
		// 如果是窗口计算，最大时间就是Now，否则就是MaxTime
		// todo 二者为什么要区分？，统一now是否可行
...
// compileFields 对Field部分处理（就是select语句的projection部分）
func (c *compiledStatement) compileFields(stmt *influxql.SelectStatement) error {
	...
	for _, f := range stmt.Fields {
		...
		// 如果time使用了别名，绑定到全局的TimeFieldName上
...
// 创建执行表达式的节点，并将该节点连接到WriteEdge作为输出。
// 对field进行属性提取，如果是函数则进行参数校验等
...
func (c *compiledStatement) compileDimensions(stmt *influxql.SelectStatement) error {
	...
	for _, d := range stmt.Dimensions {
		...
		case *influxql.Call:
			...
			// 只能存在time()函数调用，并且它有一个或两个duration参数。 就这么粗暴处理？？
			if expr.Name != "time" {
...
// 验证字段是否相互兼容。它在编译结束但链接之前运行。
// 就是根据compile所产生的属性，再进行全局的关联检测，确定执行上是否兼容
